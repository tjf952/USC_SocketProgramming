...
string search(string str){
	...
	cout << "The ServerA has found < " << num << 
...
string prefix(string str){
	...
	cout << "The ServerA has found < " << cnt << " > matches" << endl;
...
int main(void)
{
	...
	if(( error = getaddrinfo("localhost", APORT, &serv_info, &servinfo) ) != 0){
	...
	cout << "\nThe ServerA is up and running using UDP on port " << APORT << "." << endl;
	...
	while(1){
		...
		string input = msg;
		cout << "The ServerA received input < " << input << " > and operation < " 
			<< function << " >" << endl;

		// Get results in a string
		string temp = "A :: " + func(function, input);
		...
		cout << "The ServerA finished sending the output to AWS\n" << endl;
	}
